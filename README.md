...
- PDF 阅读标注：支持按视觉行与分栏重建的文字框选、高亮、下划线、文字批注、区域标注、标注导航、快捷键、结构化标注导入/导出和带标注 PDF 副本导出；复杂 PDF 的节点错序不会再导致选区跳到上一段或漏掉同一行右侧片段。
...
- 阅读器使用接近全屏的工作区，可在“上下连续滚动”和“单页翻页”之间切换。先启用工具，再拖选文字即可自动保存标注；标注模式不会同时触发翻译。
