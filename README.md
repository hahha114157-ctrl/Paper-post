...
- 阅读翻译：单击英文单词或框选英文段落后使用浏览器本地英译中，可将译文加入论文笔记或生词本。

个人数据只保存在当前浏览器的 `localStorage`，不会上传到 GitHub。清理浏览器数据或更换设备前请先导出备份。

## 阅读翻译

- 顶部“译”按钮可以总开关阅读翻译，并分别控制单词点击、选区自动翻译和本地缓存。
- 翻译仅在论文摘要、详情和文献库阅读文本中生效，不会抢占标题链接、按钮或输入框。
- 优先使用浏览器内置 `Translator API` 完成英译中；首次使用可能需要确认下载语言包。
- 当前版本不接入云端翻译、不保存第三方 API 密钥。不支持本地翻译的浏览器会明确提示，并允许复制原文。
- 生词本保存在个人文献库中；译文也可以附带原文写入对应论文笔记。
